...
Starfield::Starfield(const sf::Vector2f size, const unsigned int numberOfStars, const sf::Color color)
    ...
    , m_movemultiplier(numberOfStars)
	, m_size(size)
	, m_color(color)
    , m_alpha(numberOfStars)
    , m_maxaplha(numberOfStars)
    , m_right(numberOfStars)
    , m_down(numberOfStars)
    , m_ma(numberOfStars,100)
{
	randomSeed();
	regenerate();
}
...
void Starfield::move(float movement)
{
    float fa=1+(0.003*movement);
    int si=(int)m_vertices.size();
    for(int i=0;i<si;i++)
    {
        if(m_alpha[i]<m_maxaplha[i])
            m_alpha[i]*=fa,
            m_vertices[i].color.a=m_alpha[i],
            m_ma[i]=m_vertices[i].color.a*movement;
        m_vertices[i].position.x+=(m_right[i]?-m_ma[i]:m_ma[i])*m_movemultiplier[i].x;
        m_vertices[i].position.y+=(m_down[i]?-m_ma[i]:m_ma[i])*m_movemultiplier[i].y;
        m_movemultiplier[i]*=1.003f;
        if(m_vertices[i].position.x<0||m_vertices[i].position.x>m_size.x||m_vertices[i].position.y<0||m_vertices[i].position.y>m_size.y)
            m_movemultiplier[i].x=randomValue(0.00004,0.006),
            m_movemultiplier[i].y=0.006-m_movemultiplier[i].x,
            m_vertices[i].position.x=m_size.x/2+randomValue(-1,1),
            m_vertices[i].position.y=m_size.y/2+randomValue(-1,1),
            m_vertices[i].color.a=0,
            m_right[i]=m_vertices[i].position.x<m_size.x/2,
            m_down[i]=m_vertices[i].position.y<m_size.y/2,
            m_alpha[i]=2.55,
            m_maxaplha[i]=250;
    }
}
...
void Starfield::regenerate()
{
    ...
    for (auto& movemultiplier :m_movemultiplier)
    {
        movemultiplier.x = randomValue(0.00004,0.006);
        movemultiplier.y = 0.006-movemultiplier.x;
    }
}
...
void Starfield::regenerate(const sf::Vector2f size, const unsigned int numberOfStars)
{
    ...
    m_movemultiplier.resize(numberOfStars);
    m_alpha.resize(numberOfStars);
    m_maxaplha.resize(numberOfStars);
    m_right.resize(numberOfStars);
    m_down.resize(numberOfStars);
    m_ma.resize(numberOfStars,100);
	regenerate(size);
}
